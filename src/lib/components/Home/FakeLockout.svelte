<script lang="ts">
    ...
    editor.subscribe((e) => {
        if(e) {
            ...
            // e.setValue('#include <bits/stdc++.h>\nint main() {\n\tcout<<"Hello World!"<<endl;\n}'.slice(0, scroll > 3*height && scroll < 4*height ? Math.floor((scroll - 3*height)/height)*64 : 64));
        }
    });
    // $: console.log(scroll > 4*height && scroll < 6*height ? Math.floor((scroll - 4*height)/(2*height)*69) : 69);
    // $: console.log('#include <bits/stdc++.h>\nint main() {\n\tcout<<"Hello World!"<<endl;\n}'.slice(0, scroll > 4*height && scroll < 6*height ? Math.floor((scroll - 4*height)/(2*height)*69) : 69));
    ...
    $: if($editor) {
        $editor.setValue(
            ...
            .slice(
                ...
                Math.floor((scroll - 4*height)/(2*height)*69) :
                0
            )
        )
    }
